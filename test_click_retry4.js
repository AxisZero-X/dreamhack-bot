...
async function run() {
  ...
  if (state.retryBtnExists) {
      console.log('Clicking Retry via Native click() directly on ElementHandle');
      ...
      if(retryEl) {
         await retryEl.click(); // Puppeteer element click
         await randomDelay(2000, 3000);
      }
      ...
      console.log('State after native ElementHandle Retry clicked:', stateAfterRetry);
  }
