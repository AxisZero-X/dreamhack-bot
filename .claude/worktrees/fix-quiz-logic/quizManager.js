...
/**
 * DOM 요소에 직접 이벤트를 발생시켜 클릭을 에뮬레이션
 */
async function emulateClick(page, elementHandle) {
  if (!elementHandle) return;
  await page.evaluate((el) => {
    // 요소가 화면에 보이도록 스크롤
    el.scrollIntoView({ behavior: 'smooth', block: 'center' });

    // React/Vue 등의 프레임워크가 이벤트를 감지할 수 있도록 여러 이벤트 발생
...
async function solveQuiz(page, cursor) {
  ...
  for (let step = 0; step < totalSteps; step++) {
    ...
    for (let c = 0; c < choiceCount; c++) {
      ...
      // cursor.click 대신 직접 DOM 이벤트 발생
      await emulateClick(page, choiceHandle);
      ...
      if (submitBtn) {
        await emulateClick(page, submitBtn);
        logger.info('  📤 확인 클릭');
      }
...
async function clickNextStep(page, cursor, nextIndex) {
  ...
  if (steps[nextIndex]) {
    await emulateClick(page, steps[nextIndex]);
